...
export const submitAnswer = async (vid: number, qid: number, cid: number) => {
    try {
        const token = localStorage.getItem('authToken');
        if (!token) {
            throw new Error('Token is missing');
        }
        const response = await axios.post<ApiResponse<SubmitResultDto>>(`${API_BASE_URL}/question/submit`, {
            vid: vid,
            qid: qid,
            cid: cid
        },{
...
export const getQuestions = async (vid: number): Promise<Question[]> => {
    try {
        const token = localStorage.getItem('authToken'); // 从 localStorage 获取 token
        if (!token) {
            throw new Error('Token is missing');
        ...
        // 发送 POST 请求
        const response = await axios.post<ApiResponse<Question[]>>(`${API_BASE_URL}/question/show`, body, config);
...
export const getChoices = async (qid: number): Promise<Choice[]> => {
    try {
        const token = localStorage.getItem('authToken'); // 从 localStorage 获取 token
        if (!token) {
            throw new Error('Token is missing');
        ...
        // 发送 POST 请求
        const response = await axios.post<ApiResponse<Choice[]>>(`${API_BASE_URL}/question/showchoices`, body, config);
...
export const setVideoState = async (videoId: number, state: number) => {
    try {
        const token = localStorage.getItem('authToken');
        ...
        const body: { videoId: number; state: number } = {videoId, state};
